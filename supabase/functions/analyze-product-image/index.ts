...
serve(async (req) => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      console.error("AI gateway error", response.status, lastErrText);
      ...
      // 429 — wait with exponential backoff (1.5s, 3s, 6s, 12s)
    ...
    if (!response || !response.ok) {
      return new Response(
        JSON.stringify({ error: "Rate limited by AI gateway. Please retry in a moment." }),
